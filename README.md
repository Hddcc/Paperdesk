...
| 模块 | 内容 |
| --- | --- |
| 📚 [1. 系统功能介绍](#1-系统功能介绍) | 论文库、问答、报告、分类、阅读入口 |
| 🚀 [2. 快速使用](#2-快速使用) | 环境、下载、配置、启动、校验 |
| 🧭 [3. 项目结构](#3-项目结构) | 目录结构、运行结构视角 |
| 🤖 [4. Agent 架构介绍](#4-agent-架构介绍) | 路由、上下文、工具调用、扩展能力 |
| 📮 [5. 联系方式](#5-联系方式) | 联系方式 |

<details open>
<summary>📚 <a href="#1-系统功能介绍">1. 系统功能介绍</a></summary>

- [1.1 本地论文库管理](#11-本地论文库管理)
- [1.2 AI 论文问答与分析](#12-ai-论文问答与分析)
- [1.3 报告保存与-markdown-导出](#13-报告保存与-markdown-导出)
- [1.4 论文分类与标签整理](#14-论文分类与标签整理)
- [1.5 PDF 阅读与多页面入口](#15-pdf-阅读与多页面入口)

</details>

<details>
<summary>🚀 <a href="#2-快速使用">2. 快速使用</a></summary>

- [2.1 环境要求](#21-环境要求)
- [2.2 下载项目](#22-下载项目)
- [2.3 配置后端环境变量](#23-配置后端环境变量)
- [2.4 启动后端](#24-启动后端)
- [2.5 配置并启动前端](#25-配置并启动前端)
- [2.6 推荐使用流程](#26-推荐使用流程)
- [2.7 常用校验命令](#27-常用校验命令)

</details>

<details>
<summary>🧭 <a href="#3-项目结构">3. 项目结构</a></summary>

- [3.1 目录结构](#31-目录结构)
- [3.2 运行结构视角](#32-运行结构视角)

</details>

<details>
<summary>🤖 <a href="#4-agent-架构介绍">4. Agent 架构介绍</a></summary>

- [4.1 Agent 总体设计](#41-agent-总体设计)
- [4.2 路由决策与运行路径](#42-路由决策与运行路径)
- [4.3 上下文与记忆管理](#43-上下文与记忆管理)
- [4.4 Agent Loop 与工具调用](#44-agent-loop-与工具调用)
- [4.5 报告沉淀与研究任务](#45-报告沉淀与研究任务)
- [4.6 扩展能力](#46-扩展能力)

</details>

<details>
<summary>📮 <a href="#5-联系方式">5. 联系方式</a></summary>
...
PaperDesk 也可以作为普通 AI 助手使用。用户没有要求基于论文库、上传论文、所选论文或文献证据回答，也没有写操作和安全约束时，系统会按普通问答自然回复，并尽量跳过完整路由判断；当用户选择论文，或者提到论文库、标签、分类、报告时，系统会进入论文库能力流程。其中纯粹的数量、页数、标题、作者、标签列表等只读字段查询，会优先通过本地确定性读取返回；涉及解释、分析、对比、报告、后续处理或写操作的混合请求，仍会进入正常 Agent 链路。
...
FastPath / 意图识别 / 路由决策 / 作用范围解析 / 工具调度 / 写操作确认 / Trace
...
PaperDesk 在每轮对话开始时会先做一层保守的本地判断，再决定是否进入完整路由。系统会判断用户是在普通问答、论文问答、整理标签、修改分类、保存报告，还是在纠正上一轮回答。这个判断会影响后续是否读取论文库、是否调用工具、是否需要确认，以及最终回答应该采用普通回答、确定性只读回答还是证据型回答。

系统会结合规则判断和大模型判断来选择处理路径：规则用于保护高风险场景，大模型用于结合上下文、用户问题和可用能力判断更合适的处理方式。最终系统会在两者之间做裁决，让普通问题保持灵活，让危险操作保持稳妥。

#### 4.2.1 路由流程

进入完整路由时，系统会读取当前用户问题、已选论文、附件、最近对话、用户偏好和可用工具。规则会优先检查删除、清空、移除、覆盖、重建、批量修改、纠错、选中文档问答和论文库写入这类强约束场景；如果命中了这些场景，系统会把请求送入工具流程、确认流程或纠错流程。

如果请求被识别为保守的普通聊天，系统会跳过 LLM Router 直接走普通回答；如果请求是纯粹的论文库数量、论文页数、标题、作者或标签统计等只读字段查询，系统会直接读取本地数据并生成模板回答。
...
普通概念问题、历史常识、编程解释、使用建议和不依赖论文库的问题会走直接回答路径。对没有论文库指代、没有选中文档、没有写操作和安全约束的普通聊天，系统会跳过完整 LLM Router，直接生成自然回答。比如“RAG 是什么”“怎么阅读一篇论文”这类问题，可以直接结合上下文回答。

纯粹的论文库状态、论文元数据和标签分类读取，会优先走确定性只读路径。它会读取 SQLite 中的论文、分类和 chunk 元数据，并记录真实只读 trace；如果同一句里还包含解释、分析、对比、报告、后续处理或写操作，系统会回到正常 Agent 流程。
...
Agent Loop 负责处理需要工具推理或多步执行的用户目标。普通聊天和纯只读元数据/标签查询可以在进入 Loop 前完成；其他论文库问题会由 Loop 根据问题选择下一步，调用论文库、检索、分类、报告或记忆相关工具，拿到工具结果后再判断是否继续检索、补查证据、生成回答或进入确认流程。
...
Observation 是工具调用或确定性读取后的观察结果。ReAct 工具调用会产生 Observation；确定性只读路径也会记录 deterministic observation 和等价只读 tool trace。它会告诉 Agent 或后续排查流程工具是否成功、查到了哪些论文、匹配到哪些标签、检索到了多少证据、是否需要继续操作，以及是否出现了错误。
