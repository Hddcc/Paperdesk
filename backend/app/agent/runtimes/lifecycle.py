...
from app.models import AgentLifecycleStage, PaperDeskRoute, PaperDeskRuntimeKind, RuntimeMetricsEnvelope, RuntimeRequest, RuntimeResult
...
class LifecycleRuntime:
    ...
    def handle(self, request: RuntimeRequest) -> RuntimeResult:
        ...
        if self.implementation is not None:
            ...
            return result
        status, message = self._fallback_status_and_message(request)
    ...
    def _fallback_status_and_message(self, request: RuntimeRequest) -> tuple[str, str]:
        if request.route.route in {PaperDeskRoute.WRITE_PENDING, PaperDeskRoute.WRITE_CONFIRMED}:
            return "pending_confirmation", "write runtime requires explicit scope and confirmation before execution"
    ...
    def _execution_policy(self, request: RuntimeRequest) -> dict[str, Any]:
        return default_runner_policy_for_runtime(
            self.runtime_kind,
            request.route.orchestration_pattern,
        ).as_trace_payload()
