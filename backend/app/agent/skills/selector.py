...
class SkillSelector:
    ...
    def select(
        ...
        *,
        ...
        active_capability_id = (capability_id or "").strip().casefold()
        skills = {
            skill.skill_id: skill
            for skill in (available_skills or [])
            if self._is_default_candidate(skill)
            and self._capability_matches(skill, active_capability_id)
        }
        if not skills:
        ...
        matched_signals: dict[str, Any] = {
            ...
            "capability_id": active_capability_id or None,
        }
        cleaned_signals = {
            key: value
    ...
    @staticmethod
    def _capability_matches(skill: SkillManifest, capability_id: str) -> bool:
        ...
        declared = {
            ...
            for item in [
                ...
                *((skill.trigger.capability_ids if skill.trigger is not None else []) or []),
            ]
            if item
        }
        if not declared:
            return True
        return capability_id in declared or "shared" in declared or "common" in declared
